...
def generate_topics_with_gemini(articles):
    ...
    prompt = f"""You are an expert political analyst and journalist. Your task is to analyze political news articles and intelligently group them into coherent political topics.

ANALYSIS RULES FOR POLITICAL NEWS:
- Group articles that discuss the same political event, announcement, scandal, or policy development
- Look for common political entities (politicians, parties, institutions, laws) and themes
...
POLITICAL GROUPING CRITERIA:
1. Same politician or political figure (speeches, controversies, appointments)
2. Same political party or coalition (strategies, internal conflicts, announcements)
3. Same policy or legislation (laws, reforms, regulatory changes)
4. Same political scandal or controversy (investigations, legal proceedings)
5. Same election or electoral process (campaigns, results, analyses)
6. Same institutional decision (parliament, government, courts)
7. Same international relations or diplomacy events
8. Same social or economic policy debates

OUTPUT FORMAT - Return only this JSON structure:
{{
  "topics": [
    {{
      "name": "Concise political topic title (max 8 words)",
      "summary": "2-3 sentence journalistic summary combining all articles with political context",
      "description": "One compelling sentence describing the political significance",
      "article_urls": ["url1", "url2", ...],
      "keywords": ["politician_name", "party", "policy_area"],
      "category": "One of: National, International, Elections, Policy, Scandal, Parliament"
    }}
  ]
}}

POLITICAL ARTICLES TO ANALYZE:
...
"""
    ...
    try:
        print(f"🧠 [GEMINI] ===== SENDING POLITICAL ANALYSIS REQUEST =====", flush=True)
        ...
        print(f"🧠 [GEMINI] ✅ Political analysis completed in {end_time - start_time:.2f} seconds", flush=True)
        
        if not response.text:
            print("🧠 [GEMINI] ❌ ERROR: Empty response from Gemini", flush=True)
            return []
        
        print(f"🧠 [GEMINI] ===== GEMINI POLITICAL ANALYSIS RESPONSE =====", flush=True)
        ...
        try:
            ...
            print(f"🧠 [GEMINI] ✅ Political analysis JSON parsed successfully", flush=True)
        ...
        print(f"🧠 [GEMINI] Found {len(gemini_topics)} political topics", flush=True)
        ...
        print(f"🧠 [GEMINI] ===== POLITICAL ANALYSIS COMPLETE =====", flush=True)
        print(f"🧠 [GEMINI] Political topics created: {topics_with_articles}", flush=True)
...
def main():
    ...
    print(f"🚀 [SCRAPER] ===== STARTING POLITICAL NEWS SCRAPER =====", flush=True)
    ...
    if args.all:
        print(f"🚀 [SCRAPER] Mode: FETCH ALL POLITICAL ARTICLES", flush=True)
    else:
        print(f"🚀 [SCRAPER] Mode: FETCH NEW POLITICAL ARTICLES ONLY", flush=True)
    ...
    print(f"🚀 [SCRAPER] Starting parallel political news scraping: {len(tasks)} sources", flush=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_task, timeout=300):
            ...
            print(f"PROGRESS:{progress:.2f}:Political Sources", flush=True)

    print(f"🚀 [SCRAPER] Political scraping complete: {len(all_articles)} articles collected", flush=True)
    ...
    print(f"🚀 [SCRAPER] Unique political articles: {len(unique_articles)}", flush=True)
    ...
    if unique_articles:
        ...
        print(f"🧠 [GEMINI] Starting political topic generation with {len(unique_articles)} articles...", flush=True)
        topics = generate_topics_with_gemini(unique_articles)
        
        remaining_articles = [
            article for article in unique_articles 
            if not any(article["url"] in [a["url"] for a in topic["articles"]] for topic in topics)
        ]
...
"""
        ...
        print(f"✅ [COMPLETE] Political news analysis complete!", flush=True)
        print(f"✅ [COMPLETE] Topics: {len(topics)}, Articles: {len(unique_articles)}, Remaining: {len(remaining_articles)}", flush=True)
